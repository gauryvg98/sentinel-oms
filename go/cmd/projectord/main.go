// Command projectord copies the journal into Supabase.
//
...
func main() {
	...
	if len(os.Args) > 1 && os.Args[1] == "schema" {
		...
		os.Stdout.WriteString(supabase.Schema)
		return
	}
	...
	if errors.Is(err, supabase.ErrNotConfigured) {
		// Running without a projector is a deployment choice; running with a
		// broken one is a fault. Saying which is which saves an hour.
		log.Fatal("projectord: SUPABASE_URL and SUPABASE_SERVICE_KEY are not set")
	}
	if err != nil {
	...
	batch := make([]supabase.Row, 0, batchSize)
	var highest uint64
...
func flush(client *supabase.Client, cursor *journal.Cursor, batch []supabase.Row, highest uint64) bool {
	if len(batch) == 0 {
		return true
	}
...
func toRow(frame *journal.Record) (supabase.Row, bool) {
	row := supabase.Row{
